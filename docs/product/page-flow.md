# AI 热点信息每日汇总系统页面流程

## 公开首页日期筛选流程（2026-09-14）

访问公开首页 → 默认加载北京时间当天已发布 digest → 点击日期选择器 → 选择过去日期 → 调用公开日期接口 → 加载所选日期已发布 digest → 替换首页简报内容；没有对应 digest 时展示空状态，未来日期不可选择。

## 自然语言信息库聊天流程（2026-09-08）

登录后任意主页面 → 打开右下角悬浮信息库助手 → 加载用户最近会话与消息历史 → 输入自然语言 → `LibraryChatIntentClassifier` 判断是否属于信息库查询 → 范围外问题返回固定拒答 → 范围内问题调用默认 LLM Provider + DeepAgents 查询 Agent → Agent 通过 `search_library_database` 只读工具查询已入库内容 → SSE 流式展示状态、回答文本、结果预览和筛选 chips → 打开原文，或跳转信息库查看全部。LLM 不可用时降级为原句关键词检索，整个流程不触发外部实时检索。详见 [自然语言信息库检索需求](natural-language-library-search.md)。

## 收藏流程（2026-09-06）

信息库/条目详情/关注流 → 收藏图标 → 无目录时直接存根目录；有目录时选择目录 → 确认收藏 → 我的收藏。已收藏图标打开管理弹窗，可移动或取消收藏。我的收藏 → 目录菜单 → 新建/重命名/删除；删除目录确认后，其内容移至根目录。详情抽屉关闭后保留当前目录、筛选和页码。详见 [收藏设计](favorites.md)。

## 公开今日简报流程（2026-09-09）

访问站点 → 判断是否已登录 → 未登录展示公开今日简报 → 读取当天已发布 digest → 展示概览、重点条目、精选信息流 → 用户可阅读条目卡片并打开原文 → 用户选择过去日期 → 读取所选日期已发布 digest → 替换首页内容 → 用户点击登录 → 进入登录页 → 登录成功后进入今日简报工作区。公开页面不加载收藏、关注、信息库聊天和管理功能。

## 登录后访问流程（2026-09-09）

访问私有页面 → 判断是否已登录 → 未登录跳转 `/login?next=<原路径>` → 登录成功 → 恢复原目标页面；若没有目标页面则进入 `/today` → 根据角色展示普通用户导航或普通用户加管理员导航。

版本：v0.3

日期：2026-09-09

阶段：产品设计

关联文档：

- [PRD](prd.md)
- [产品原型](product-prototype.md)

## 1. 页面地图

```mermaid
flowchart TD
    Home[公开首页 / 今日简报只读] --> Login[登录页]
    Home --> PublicOriginal[打开原文]
    Login --> Today[今日简报工作区]
    Today --> Detail[详情抽屉]
    History[历史简报] --> Detail
    Library[信息库] --> Detail
    FloatingChat[信息库悬浮聊天助手] --> Library
    Library --> SaveSearch[保存当前搜索]
    Library --> Analytics[数据概览图表]
    Analytics --> LibraryFilter[反向更新筛选条件]
    SaveSearch --> Following[我的关注]
    Today --> Following
    Following --> Today
    AdminSources[来源管理] --> SourceForm[新增/编辑来源表单卡片]
    AdminJobs[任务日志] --> JobDetail[任务详情]
    AdminLLM[LLM 设置] --> LLMForm[新增/编辑 Provider 表单卡片]
    AdminUsers[用户管理] --> UserForm[新增/编辑用户表单卡片]
    PrivateRoute[访问私有路由] --> AuthGate{是否已登录}
    AuthGate -- 否 --> Login
    AuthGate -- 是 --> Today
```

## 1.1 自然语言悬浮聊天流程

```mermaid
flowchart TD
    A[登录后进入任意主页面] --> B[打开信息库悬浮聊天助手]
    B --> C[加载最近会话与消息历史]
    C --> D[输入自然语言查询]
    D --> E{输入是否合法}
    E -- 否 --> F[展示输入校验提示]
    E -- 是 --> G[保存用户消息]
    G --> H{是否属于信息库查询}
    H -- 否 --> I[保存并流式返回固定拒答]
    H -- 是 --> J[调用默认 LLM Provider + DeepAgents]
    J --> K{LLM 输出是否有效}
    K -- 是 --> L[调用 search_library_database 查询已入库内容]
    K -- 否 --> M[降级为原句关键词搜索]
    L --> N[SSE 返回 delta 与 results]
    M --> N
    N --> O{是否有结果}
    O -- 是 --> P[聊天气泡展示结果预览]
    O -- 否 --> Q[展示站内无结果空状态]
    P --> R{用户操作}
    R -- 打开原文 --> S[外部原文]
    R -- 查看全部 --> T[跳转信息库并回填筛选]
    Q --> U[提示仅搜索已入库内容]
```

## 2. 登录与访问流程

```mermaid
flowchart TD
    A[访问站点] --> B{是否已登录}
    B -- 否 --> C[公开今日简报]
    B -- 是 --> D[今日简报工作区]
    C --> E{用户操作}
    E -- 阅读/详情/原文 --> C
    E -- 点击登录 --> F[登录页]
    F --> G[输入账号密码]
    G --> H{认证成功}
    H -- 否 --> I[展示错误提示]
    I --> F
    H -- 是 --> D
    D --> J{用户角色}
    J -- 普通用户 --> K[展示普通用户导航]
    J -- 管理员 --> L[展示普通用户导航和管理员导航]
```

## 3. 今日简报浏览流程

```mermaid
flowchart TD
    A[进入今日简报] --> B[默认展示系统精选]
    B --> C{切换视图}
    C -- 系统精选 --> D[按全局 score 展示]
    C -- 我的关注 --> E[应用用户规则计算 user score]
    D --> F[浏览今日概览/重点专题/最新信息流]
    E --> F
    F --> G{用户操作}
    G -- 查看详情 --> H[打开详情抽屉]
    G -- 打开原文 --> I[跳转外部原文]
    G -- 多看类似 --> J[记录正向反馈]
    G -- 少看类似 --> K[记录负向反馈]
    G -- 屏蔽来源 --> L[二次确认后屏蔽来源]
    H --> F
    J --> E
    K --> E
    L --> E
```

## 4. 信息库搜索与保存搜索流程

```mermaid
flowchart TD
    A[进入信息库] --> B[输入关键词]
    B --> C[选择来源/来源类型/分类/状态/时间范围]
    C --> D[检索已采集入库内容]
    D --> E[同步加载数据概览]
    E --> F{是否有结果}
    F -- 是 --> G[展示结果列表和详情面板]
    F -- 否 --> H[展示站内无结果空状态]
    G --> I{用户操作}
    I -- 查看详情 --> J[打开详情抽屉]
    I -- 保存当前搜索 --> K[保存到我的关注]
    I -- 点击图表项 --> L[反向写入来源类型/分类/最低分]
    L --> D
    K --> M[参与我的关注流排序和过滤]
```

## 5. 我的关注配置流程

```mermaid
flowchart TD
    A[进入我的关注] --> B[编辑关注关键词]
    B --> C[编辑排除关键词]
    C --> D[选择关注分类]
    D --> E[选择关注来源类型]
    E --> F[编辑屏蔽来源]
    F --> G[查看保存搜索]
    G --> H[查看反馈记录]
    H --> I[保存设置]
    I --> J[应用排除关键词/屏蔽来源/关闭来源类型硬过滤]
    J --> K[应用关注分类/来源类型默认筛选]
    K --> L[应用关注关键词/分类/反馈软加权]
    L --> M[规则应用到当前用户的我的关注视图]
```

## 6. 详情抽屉流程

```mermaid
flowchart TD
    A[从简报/历史/信息库点击详情] --> B[打开详情抽屉]
    B --> C{是否属于 topic}
    C -- 是 --> D[展示专题摘要/相关来源/事件时间线]
    C -- 否 --> E[展示单条内容详情]
    D --> F{摘要状态}
    E --> F
    F -- 成功 --> G[展示中文摘要和为什么重要]
    F -- 失败 --> H[展示失败状态和原始摘要兜底]
    G --> I[打开原文或提交反馈]
    H --> I
```

## 7. 管理员来源管理流程

```mermaid
flowchart TD
    A[管理员进入来源管理] --> B{操作}
    B -- 新增 --> C[填写新增来源表单卡片]
    B -- 编辑 --> D[切换到编辑来源表单卡片]
    B -- 启用/禁用 --> E[更新来源状态]
    B -- 手动抓取 --> F[触发单 source 抓取]
    C --> G[保存配置]
    D --> G
    G --> H[更新来源列表]
    F --> I{抓取成功}
    I -- 是 --> J[更新最近抓取时间]
    I -- 否 --> K[显示最近错误]
```

## 8. 管理员任务流程

```mermaid
flowchart TD
    A[管理员进入任务日志] --> B[筛选任务]
    B --> C[查看任务列表]
    C --> D{任务状态}
    D -- 成功 --> E[查看详情]
    D -- 失败 --> F[查看错误详情]
    F --> G{是否重跑}
    G -- 是 --> H[创建新的任务记录]
    H --> I[执行任务]
    I --> J[刷新任务状态]
    G -- 否 --> C
```

## 9. 管理员配置 LLM 流程

```mermaid
flowchart TD
    A[管理员进入 LLM 设置] --> B{操作}
    B -- 新增 --> C[填写 provider 配置]
    B -- 编辑 --> D[修改 provider 配置]
    B -- 测试 --> E[调用测试连接]
    B -- 设置默认 --> F[更新默认 provider]
    C --> G[保存]
    D --> G
    E --> H{测试成功}
    H -- 是 --> I[显示可用]
    H -- 否 --> J[显示错误原因]
```

## 10. 管理员用户管理流程

```mermaid
flowchart TD
    A[管理员进入用户管理] --> B{操作}
    B -- 新增用户 --> C[填写邮箱/用户名/初始密码/角色]
    B -- 编辑角色 --> D[修改用户角色]
    B -- 启用/禁用 --> E[修改用户状态]
    B -- 重置密码 --> F[设置新密码]
    C --> G[保存并加密密码]
    D --> H[更新用户信息]
    E --> H
    F --> H
    G --> H
    H --> I[刷新用户列表]
```
