...
- 信息库智能助手：全局悬浮聊天框基于 assistant-ui core primitives，支持自然语言查询已入库 AI 信息，使用意图门控、DeepAgents 只读数据库工具、SSE 流式返回、用户级聊天历史和历史会话删除。
...
| 模块 | 技术 |
| --- | --- |
| 前端 | Next.js App Router, React, TypeScript, assistant-ui core primitives, shadcn-style local UI primitives, Apache ECharts, TanStack Table, lucide-react |
| 后端 | FastAPI, Python 3.12, DeepAgents |
| 架构 | DDD 风格模块化单体 |
| 数据库 | PostgreSQL |
| 缓存/任务状态 | Redis |
| 调度 | APScheduler |
